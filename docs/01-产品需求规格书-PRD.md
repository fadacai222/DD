...
- 不把 Bot Token 暴露客户端。
- 当前静态 WebP 为最低稳定支持；TGS/WebM 后续扩展。
...
## 4.14 二维码（SERVER-AUTO-VERIFIED / FLUTTER-KNOWN-FAILURE）
...
当前服务端 PostgreSQL/HTTP/OpenAPI 已自动验证；Flutter 页面与入口代码存在，但 2026-08-11 当前 analyzer/Widget/API tests 有已知失败，修复前不得发布。
