...
void main() {
  test('iOS project keeps DD platform foundation contracts', () {
    ...
    expect(
      project,
      contains('PRODUCT_BUNDLE_IDENTIFIER = org.openimx.client;'),
    );
    ...
    expect(
      ...
      reason:
          'Runner project Debug/Profile/Release deployment targets must stay aligned',
    );
  ...
  test('iOS secure storage stays on native flutter_secure_storage path', () {
    ...
    final gateway = File(
      'lib/core/security/dd_secure_storage.dart',
    ).readAsStringSync();
    final vault = File(
      'lib/features/auth/data/auth_session_vault.dart',
    ).readAsStringSync();
  ...
  test('native services register outside AppDelegate', () {
    ...
    for (final service in const [
      ...
      expect(registrar, contains('register($service.self, with: registry)'));
      expect(project, contains('$service.swift in Sources'));
    }
  ...
  test(
    'AppDelegate forwards iOS push callbacks without cutting Flutter delegates',
    () {
      ...
      expect(
        appDelegate,
        contains(
          'PushNotificationService.shared.didFailToRegisterForRemoteNotifications',
        ),
      );
      expect(appDelegate, contains('foregroundPresentationOptions'));
      expect(appDelegate, contains('didReceiveNotificationResponse'));
      expect(appDelegate, contains('super.application('));
      expect(appDelegate, contains('super.userNotificationCenter('));
    },
  );
}
